...
programa {
  funcao inicio() {
    ...
    inteiro diasAlbergue
    real valorDiaria, desconto = 0.25, multa = 150, totalBruto, totalDesconto, valorFinal

    escreva("Quantos dias vai passar no Albergue? \n: ")
    leia (diasAlbergue)

    se (diasAlbergue <= 5){
    valorDiaria = diasAlbergue * 100
    }
    se(diasAlbergue >= 6 e diasAlbergue <= 10){
      valorDiaria = diasAlbergue * 90
    }
    se(diasAlbergue >= 11){
      valorDiaria = diasAlbergue * 80
    }

    totalBruto = multa + valorDiaria
    totalDesconto = (totalBruto * desconto)
    valorFinal = totalBruto - totalDesconto

    escreva ("\n===================================================\n")
    escreva ("Valor das diárias: R$ ", valorDiaria)
    escreva ("\nMulta: R$ ", multa)
